...
We also tested other potential interactions (Age × Serum Creatinine, EF × Serum Creatinine, Sex × Smoking), but none were statistically significant:

```{r interaction-summary}
# Test other interactions
cox_interaction_age_creat <- coxph(
  surv_obj ~ age * serum_creatinine + sex + ejection_fraction + 
    high_blood_pressure + diabetes + smoking,
  data = heart
)

cox_interaction_ef_creat <- coxph(
  surv_obj ~ ejection_fraction * serum_creatinine + age + sex + 
    high_blood_pressure + diabetes + smoking,
  data = heart
)

cox_no_smoking_int <- coxph(
  surv_obj ~ sex + smoking + age + ejection_fraction + serum_creatinine + 
    high_blood_pressure + diabetes,
  data = heart
)

cox_interaction_sex_smoking <- coxph(
  surv_obj ~ sex * smoking + age + ejection_fraction + serum_creatinine + 
    high_blood_pressure + diabetes,
  data = heart
)
...
interaction_summary <- data.frame(
  Interaction = c("Age × Ejection Fraction", 
                  "Age × Serum Creatinine", 
                  "EF × Serum Creatinine",
                  "Sex × Smoking"),
  LRT_Chisq = c(
    anova(cox_no_interaction, cox_interaction_age_ef)$Chisq[2],
    anova(cox_no_interaction, cox_interaction_age_creat)$Chisq[2],
    anova(cox_no_interaction, cox_interaction_ef_creat)$Chisq[2],
    anova(cox_no_smoking_int, cox_interaction_sex_smoking)$Chisq[2]
  ),
  p_value = c(
    anova(cox_no_interaction, cox_interaction_age_ef)$`Pr(>|Chi|)`[2],
    anova(cox_no_interaction, cox_interaction_age_creat)$`Pr(>|Chi|)`[2],
    anova(cox_no_interaction, cox_interaction_ef_creat)$`Pr(>|Chi|)`[2],
    anova(cox_no_smoking_int, cox_interaction_sex_smoking)$`Pr(>|Chi|)`[2]
  )
) %>%
  mutate(
...
knitr::kable(
  interaction_summary,
  caption = "Summary of Interaction Effect Tests",
  col.names = c("Interaction Term", "LRT Chi-square", "p-value", "Significant (α=0.05)")
)
```
